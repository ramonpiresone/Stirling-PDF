...
main() {
  ...
  # docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t frooodle/s-pdf:latest-ultra-lite -f ./Dockerfile-ultra-lite .
    ...
    #run_tests "EditMyPDF-Ultra-Lite" "./exampleYmlFiles/docker-compose-latest-ultra-lite.yml"
	#docker-compose -f "./exampleYmlFiles/docker-compose-latest-ultra-lite.yml" down
	

  #  run_tests "EditMyPDF" "./exampleYmlFiles/docker-compose-latest.yml"
 ...
 #   docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t frooodle/s-pdf:latest-ultra-lite -f ./Dockerfile-ultra-lite .
  ...
  #  run_tests "EditMyPDF-Ultra-Lite-Security" "./exampleYmlFiles/docker-compose-latest-ultra-lite-security.yml"
	#docker-compose -f "./exampleYmlFiles/docker-compose-latest-ultra-lite-security.yml" down
  #  run_tests "EditMyPDF-Security" "./exampleYmlFiles/docker-compose-latest-security.yml"
#	docker-compose -f "./exampleYmlFiles/docker-compose-latest-security.yml" down
	
	run_tests "EditMyPDF-Security-Fat" "./exampleYmlFiles/docker-compose-latest-fat-security.yml"
	if [ $? -eq 0 ]; then
		cd cucumber
		if python -m behave; then
			passed_tests+=("EditMyPDF-Regression")
		else
			failed_tests+=("EditMyPDF-Regression")
			echo "Printing docker logs of failed regression"
			docker logs "EditMyPDF-Security-Fat"
